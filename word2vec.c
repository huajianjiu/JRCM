...
real alpha = 0.025, starting_alpha, sample = 1e-3, alpha_c;
...
//    printf("%d\n", word_count);
    if (word_count - last_word_count > 10000) {
      ...
      if ((debug_mode > 1)) {
        now=clock();
        printf("%cAlpha_c: %f  Progress: %.2f%%  Words/thread/sec: %.2fk  ", 13, alpha_c,
         word_count_actual / (real)(iter * train_words + 1) * 100,
         word_count_actual / ((real)(now - start + 1) / (real)CLOCKS_PER_SEC * 1000));
        fflush(stdout);
      }
      alpha_c = starting_alpha * (1 - word_count_actual / (real)(iter * train_words + 1));
      if (alpha_c < starting_alpha * 0.0001) alpha_c = starting_alpha * 0.0001;
    }
...
//    if (word_count + ppdb_start_position > vocab_size || (word_count > vocab_size)) {
    ...
    word = ppdb_start_position + word_count;
    if (word == -1) continue;
    for (c = 0; c < layer1_size; c++) neu1[c] = 0;
    for (c = 0; c < layer1_size; c++) neu1e[c] = 0;
